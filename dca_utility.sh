...
if [ "x$DCA_MULTI_CORE_SUPPORTED" = "xyes" ]; then
    ...
    scp -i $PEER_COMM_ID -r $ARM_INTERFACE_IP:$LOG_SYNC_PATH/$SelfHealBootUpLogFile  $ARM_INTERFACE_IP:$LOG_SYNC_PATH/$PcdLogFile $ARM_INTERFACE_IP:$LOG_SYNC_PATH/$RAM_OOPS_FILE  $TMP_SCP_PATH/ > /dev/null 2>&1
    rm -f $PEER_COMM_ID
